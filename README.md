...
- В качестве файлового менеджера используется **oil.nvim** (клавиша `-`). Его
  особенностью является возможность работать со списком файлов как с буфером: переходим в режим редактирования, изменяем имя файла, сохраняем буфер, и файл переименовывается на диске. Так же если, удалить строку и сохранить, то файл будет удален, добавить — создан.
